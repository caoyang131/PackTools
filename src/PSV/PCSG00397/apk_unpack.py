...
def handlePackB(fl,fullpackname,idoffsetvalue):
    genesrtlength, = struct.unpack('<Q',fl.read(0x8))
    geneoffset = fl.tell()
    genesrtlength += geneoffset

    fileblocksize,null,files,null = struct.unpack('<4I',fl.read(0x10))
    null1 = fl.read(0x10)

    for i in range(files):
        number,null2,offset = struct.unpack('<2IQ',fl.read(0x10))
        offset += idoffsetvalue
        size,zsize = struct.unpack('<2Q',fl.read(0x8*2))
        genebreak = fl.tell()

        fullname = GENESTRT(fl,genesrtlength,number)
        packname = "pack/"+ filename + "/"
        packname += fullname

        print(packname+':'+hex(offset)+'|'+hex(zsize)+'|'+hex(size))
...
for src in srcfiles:
    ...
    while fl.tell() < filesize:
        ...
        if idstring == b'PACKHEDR' and filename == 'all.apk':
            fl.seek(start)
            #print(idstring)
            handlePackA(fl,filename)
            #break

        if idstring == b'PACKFSHD' and filename == 'fs.apk':
            fl.seek(start)
            handlePackB(fl,filename,idoffsetvalue)
    ...
    while fl.tell() < filesize:
        ...
        if idstring == b'ENDILTLE':
            ...
            if idstring == b'PACKHEDR' and filename == 'all.apk':
                ...
                handlePackA(fl,filename)
                break
            elif idstring == b'PACKFSHD' and filename == 'fs.apk':
                fl.seek(start)
                handlePackB(fl,filename,idoffsetvalue)
